...
function draw() {
  ...
  for (const p of farmPlots.values()) {
    if (p.z !== camZ) continue;
    const x = p.c * TILE - camX, y = p.r * TILE - camY;
    ctx.fillStyle = FARM_COLORS[p.stage]; ctx.fillText(FARM_GLYPHS[p.stage], x, y);
  }
  ...
  for (const job of digStairJobs.values()) {
    ...
    ctx.fillText(job.stairType === 'stair-down' ? '<' : '>', x, y);
  }
  ...
  for (const job of buildJobs.values()) {
    ...
    const ch = job.type==='stone-wall'||job.type==='dirt-wall' ? '+'
             : job.type==='stone-floor'||job.type==='dirt-floor' ? '_'
             : job.type==='stair-up' ? '<' : job.type==='stair-down' ? '>'
  ...
  for (const s of stairs.values()) {
    ...
    ctx.fillText(s.type === 'stair-up' ? '<' : '>', x, y);
  }
  ...
  if (digMode || chopMode || destroyMode || stockpileMode || buildMode || farmMode) {
    ctx.strokeStyle = digMode ? '#f80' : chopMode ? '#8f4' : destroyMode ? '#f00' : stockpileMode ? '#0af' : farmMode ? '#4a4' : '#f0f';
    ctx.fillStyle   = digMode ? 'rgba(255,136,0,0.15)' : chopMode ? 'rgba(136,255,68,0.15)' : destroyMode ? 'rgba(255,0,0,0.15)' : stockpileMode ? 'rgba(0,170,255,0.15)' : farmMode ? 'rgba(68,170,68,0.15)' : 'rgba(255,0,255,0.15)';
    ctx.lineWidth = 1;
    if (dragStartTile) {
  ...
  for (const d of dwarves) {
    ...
    if (d.carrying) {
      ctx.fillStyle = '#0af'; ctx.font = '10px monospace';
      ctx.fillText(d.carrying[0], x + TILE - 6, y);
    }
  }
}
