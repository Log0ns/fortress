...
function setMode(mode) {
  if (mode === 'build') {
    ...
    digMode = false; chopMode = false; destroyMode = false; stockpileMode = false; farmMode = false; selectMode = false; digStairMode = false;
  ...
  if (mode === 'select') document.getElementById('select-submenu').style.display = selectMode ? 'flex' : 'none';
  if (mode !== 'select' && mode !== 'cancel') { cancelMode = false; document.getElementById('select-submenu').style.display = 'none'; }
...
function updateSidebar() {
  ...
  if (!overworldView && hr >= 0 && hr < ROWS && hc >= 0 && hc < COLS) {
    ...
    const sp = stockpiles.get(tileKey(camZ,hr,hc));
  ...
  document.getElementById('btn-stone-wall').classList.toggle('active',  buildMode==='stone');
  document.getElementById('btn-dirt-wall').classList.toggle('active',   buildMode==='dirt');
  ...
  document.getElementById('btn-stair').classList.toggle('active',      buildMode==='stair');
