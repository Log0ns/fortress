...
const BUILD_TYPES = ['wall', 'floor', 'stair', 'workshop', 'door', 'bed', 'chair', 'table', 'chest', 'barrel'];

// Active material used for all build jobs
let activeMaterial = 'stone';
...
const wildlife     = new Map(); // key = id
let   _wildlifeId  = 0;

// --- Fog of war ---
const revealed = new Set();

function _isAboveSurface(z, r, c) {
  if (!surfaceZ) return true;
  return z >= surfaceZ[c][r];
}

function _revealAround(z, r, c) {
  for (const [dr, dc] of [[0,0],[-1,0],[1,0],[0,-1],[0,1]]) {
    const nr = r+dr, nc = c+dc;
    if (nr >= 0 && nr < ROWS && nc >= 0 && nc < COLS)
      revealed.add(tileKey(z, nr, nc));
  }
}

function isRevealed(z, r, c) {
  if (_isAboveSurface(z, r, c)) return true;
  if (revealed.has(tileKey(z, r, c))) return true;
  // Show walls/edges bordering revealed space
  for (const [dr, dc] of DIRS)
    if (revealed.has(tileKey(z, r+dr, c+dc))) return true;
  return false;
}
...
// Generate the starting chunk — surfaceZ and map are declared in engine.js as lets
{
  ...
  map      = chunk.map;
  currentBiome = overworld[owCurY][owCurX].biome;
  for (const [k,v] of chunk.trees) trees.set(k,v);
  // Reveal the surface layer so stone edges are visible from the start
...
const dwarves = NAMES.map((name, i) => {
  ...
  _revealAround(z, r, c);
  return { name, job: 'Idle', c, r, z, wx: owCurX, wy: owCurY, dead: false, hunger: 0, hp: 10, maxHp: 10 };
});
