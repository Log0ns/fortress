...
// --- Mode flags ---
let digMode          = false;
let digStairDownMode = false;
let digStairUpMode   = false;
...
const BUILD_TYPES = ['workshop', 'stone-wall', 'dirt-wall', 'stone-floor', 'dirt-floor', 'stair-up', 'stair-down', 'door', 'bed', 'chair', 'table', 'chest', 'barrel'];
// These require a workshop to build
const WORKSHOP_TYPES = new Set(['stone-wall','stone-floor','door','bed','chair','table','chest','barrel']);
...
function spawnFood(z, r, c)           { food.set(`${z},${r},${c}`, { z, r, c, claimedBy: null }); }
function spawnResource(z, r, c, type) { resources.set(`${z},${r},${c}`, { z, r, c, type, claimedBy: null }); }
...
const FARM_GLYPHS = ['"', ',', 'v', 'V'];
const FARM_COLORS = ['#654', '#8a6', '#4a4', '#ff0'];
...
const dwarves = NAMES.map((name, i) => {
  ...
  return { name, job: 'Idle', health: 100, c, r, z, wx: owCurX, wy: owCurY, dead: false };
});

for (let i = 0; i < 10; i++) {
  const c = 72 + i * 2;
  const r = 43;
  spawnFood(surfaceZ[c][r], r, c);
}
