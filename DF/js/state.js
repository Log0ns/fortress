...
function spawnFood(z, r, c)           { food.set(tileKey(z,r,c), { z, r, c, claimedBy: null }); }
function spawnResource(z, r, c, type) {
  // Find a free tile — prefer exact spot, then adjacent
  const candidates = [{ z, r, c }].concat(DIRS.map(([dr,dc]) => ({ z, r:r+dr, c:c+dc })));
  const tile = candidates.find(t => passable(t.z, t.r, t.c) && !resources.has(tileKey(t.z, t.r, t.c)));
  if (tile) resources.set(tileKey(tile.z, tile.r, tile.c), { z:tile.z, r:tile.r, c:tile.c, type, claimedBy: null });
}
