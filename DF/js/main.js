...
document.addEventListener('keydown', e => {
  ...
  if (e.key === 'd' || e.key === 'D') { digMode = !digMode; digStairMode = false; chopMode = false; destroyMode = false; stockpileMode = false; farmMode = false; selectMode = false; buildMode = null; updateSidebar(); return; }
...
// Release any claim on a job without deleting it, so _assignAll picks it up fresh
...
function _destroyJobForTile(z, r, c) {
  ...
  if (t === 'STONE' || t === 'DIRT') {
    ...
    destroyJobs.set(k, { z,r,c, entity:'wall', buildType: t==='STONE' ? 'stone' : 'dirt', claimedBy:null });
  }
}
...
function _canBuild(type, z, r, c) {
  ...
  if (furnitureTypes.has(type)) {
    if (t !== 'DIRT' && !floorTypes.has(t)) return 'Furniture needs a floor tile';
  } else {
    if (t !== 'AIR' && t !== 'DIRT' && !floorTypes.has(t)) return 'Tile must be open to build here';
  }
  return true;
}
...
function applyDesignation(r1, c1, r2, c2) {
  ...
  for (let r = rMin; r <= rMax; r++) {
    for (let c = cMin; c <= cMax; c++) {
      ...
      else if (digStairMode) { if ((t==='STONE'||t==='DIRT') && !digStairJobs.has(k)) digStairJobs.set(k, {z:camZ,r,c,claimedBy:null}); }
  ...
  if (dx < 4 && dy < 4) {
    ...
    if (digMode) {
      ...
      if ((t === 'STONE' || t === 'DIRT') && !digStairJobs.has(k))
        digStairJobs.set(k, { z: camZ, r, c, claimedBy: null });
...
window.togglePause  = () => { paused = !paused; updateSidebar(); };

resize();
// Center camera on dwarf spawn
